...
const deleteEntireDirectory = function deleteEntireDirectory(path) {
  if (fs.existsSync(path)) {
    fs.readdirSync(path).forEach(function (file) {
      ...
      if (fs.lstatSync(curPath).isDirectory()) deleteEntireDirectory(curPath);
...
const deleteOnlyFiles = function deleteOnlyFiles(path) {
  if (fs.existsSync(path)) {
    fs.readdirSync(path).forEach(function (file) {
      ...
      if (file.match('categories.json')) console.log('');
      else if (fs.lstatSync(curPath).isDirectory()) deleteEntireDirectory(curPath);
      else fs.unlinkSync(curPath);
    });
  }
};
